...
#threat.str <- paste(c(round(mod.Bel[[1]],3)), sep=",")  ####NEED TO FIX THIS--ONLY PRINTING THE LAST ELEMENT IN THE STATEMENT BELOW ARGH
#species.str <- paste(c(round(mod.Bel[[2]],3)), sep=",")
#pasteStr <-  paste("label=\"Simulated model ",true.model[1], "_", true.model[2],
#                   "\n Probs < ", threshold, " not drawn",
#                   "\n Threat belief= (", threat.str, ")",
#                   "\n Species belief= (", species.str, ")",
#                   "\n See Belief simulation tab for stationary belief \"", sep="")
...
#also print stationary belief-- but first convert to factroed belief form via 
writeLines(pasteStr, fileConn)
writeLines("}", fileConn)
